...
const fetchRepository = async (
  ...
  const analyzeCodebase = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("https://api.gemini.google.com/v1/analyze", { // Hypothetical Gemini API endpoint
        method: "POST",
        headers: {
    ...
    try {
      const response = await fetch("https://api.gemini.google.com/v1/analyze", { // Hypothetical Gemini API endpoint
        ...
        body: JSON.stringify({
          ...
          prompt: "Analyze this directory structure and explain the project organization to help developers and contributors understand it.",
        }),
      });
      const data = await response.json();
      setRepoData((prev) => ({ ...prev, directoryStructure: data.summary || "No analysis available" }));
    } catch (error) {
